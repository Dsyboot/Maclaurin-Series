...
from functions import InvalidCommandException
from functionsD import DecimalMath, DecimalSolver
...
expressions_history = [] # (time, expr, result, grados, range, precision)
...
def operation() -> int:
    ...
    if any(comando in funct for comando in ["/save", "/range", "/prec", "/chgrad", "/exit"]):
        ...
        if "/range" in funct:
            try:
                ...
                DecimalMath.intensity(value) # Establecer el valor de n
            except:
            ...
            try:
                # Formatear a int el valor
                value = int(funct.replace("/prec", "").strip())
                
                DecimalMath.decimal_presision(value) # Establecer el valor de la precision
            except:
                raise InvalidCommandException("El valor de la precision debe ser un entero!")
            #end try
        #end if
    else:
        # No es un comando, asi que se evalua la expresion
        solve = DecimalSolver() # Crear objeto de la clase DecimalSolver
        global ans
        ...
        if not error:
            ...
            expressions_history.append((now, funct, result, str("Radianes" if DecimalMath.inRadians else "Grados"), DecimalMath.n, DecimalMath.precision))
        #end if
        ...
        with open("history.txt", "a") as data:
            ...
            try:
                for index, vals in enumerate(expressions_history):
                    hora, func, result, grType, nVal, prec = vals
                    data.write(f"\nEcuacion: #{index + 1}\n")
                    data.write(f"Realizada con: {grType}\n")
                    data.write(f"Valor de n: {nVal}\n")
                    data.write(f"Precision decimal: {prec}\n")
...
def run(): 
    ...
    while True:
        ...
        cleardevice()
        print(f"Series de maclaurin (precision: {DecimalMath.precision}) (n: {DecimalMath.n}) (Modo: {'Radianes' if DecimalMath.inRadians else 'Grados'})\n")
        
        # Mostrar las ecuaciones y sus restricciones
        print("-"*50)
        print("Ecuaciones disponibles:")
        print("fact(x)       x es Natural que incluye 0")
        ...
        print("/prec         Cambiar la presicion decimal")
        ...
        try:
            ...
            if command == 0:
                ...
                DecimalMath.usesRadians(not DecimalMath.inRadians) # /chgrad
            elif command == 2:
                print("Cerrando...")
